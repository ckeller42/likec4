...
describe('find-relationship-paths tool', () => {
  describe('direct paths', () => {
    it('should find 1-hop direct relationship', async () => {
      ...
      const result = await handler(
        { sourceId: 'frontend', targetId: 'backend', maxDepth: 3, project: 'default' as ProjectId },
        {} as any,
      )
  ...
  describe('multi-hop paths', () => {
    ...
    it('should sort paths by length', async () => {
      ...
      const [_name, _config, handler] = findRelationshipPaths(services.likec4.LanguageServices)
      const result = await handler(
        { sourceId: 'a', targetId: 'd', maxDepth: 5, project: 'default' as ProjectId },
        {} as any,
      )
  ...
  describe('maxDepth', () => {
    it('should respect maxDepth limit', async () => {
      ...
      const [_name, _config, handler] = findRelationshipPaths(services.likec4.LanguageServices)
      const result = await handler(
        { sourceId: 'a', targetId: 'd', maxDepth: 2, project: 'default' as ProjectId },
        {} as any,
      )
    ...
    it('should find path within maxDepth', async () => {
      ...
      const [_name, _config, handler] = findRelationshipPaths(services.likec4.LanguageServices)
      const result = await handler(
        { sourceId: 'a', targetId: 'c', maxDepth: 3, project: 'default' as ProjectId },
        {} as any,
      )
  ...
  describe('edge cases', () => {
    ...
    it('should reject source equals target', async () => {
      ...
      const [_name, _config, handler] = findRelationshipPaths(services.likec4.LanguageServices)
      const result = await handler(
        { sourceId: 'frontend', targetId: 'frontend', maxDepth: 3, project: 'default' as ProjectId },
        {} as any,
      )
    ...
    it('should reject parent-child relationships', async () => {
      const { validate, services } = createTestServices()
      ...
      const [_name, _config, handler] = findRelationshipPaths(services.likec4.LanguageServices)
      const result = await handler(
        { sourceId: 'shop', targetId: 'shop.frontend', maxDepth: 3, project: 'default' as ProjectId },
        {} as any,
      )

      expect(result.isError).toBe(true)
      const content = result.content![0]!
      expect('text' in content && content.text).toContain('parent-child relationship')
    })
    ...
    it('should handle non-existent source', async () => {
      ...
      const [_name, _config, handler] = findRelationshipPaths(services.likec4.LanguageServices)
      const result = await handler(
        { sourceId: 'nonexistent', targetId: 'backend', maxDepth: 3, project: 'default' as ProjectId },
        {} as any,
      )
    ...
    it('should handle non-existent target', async () => {
      ...
      const [_name, _config, handler] = findRelationshipPaths(services.likec4.LanguageServices)
      const result = await handler(
        { sourceId: 'frontend', targetId: 'nonexistent', maxDepth: 3, project: 'default' as ProjectId },
        {} as any,
      )
  ...
  describe('relationship details', () => {
    it('should include relationship metadata in path steps', async () => {
      ...
      expect(paths).toHaveLength(1)
      expect(paths[0]!.steps[0]!.relationship.title).toBe('Calls API')
      expect(paths[0]!.steps[0]!.relationship.technology).toBe('HTTPS')
    })
  })
  ...
  describe('default maxDepth', () => {
    it('should use default maxDepth of 3 when not specified', async () => {
      ...
      const [_name, _config, handler] = findRelationshipPaths(services.likec4.LanguageServices)
      const result = await handler(
        { sourceId: 'a', targetId: 'e', project: 'default' as ProjectId },
        {} as any,
      )
  ...
  describe('multiple relationships between same elements', () => {
    it('should distinguish multiple relationships with different properties', async () => {
      ...
      const [_name, _config, handler] = findRelationshipPaths(services.likec4.LanguageServices)
      const result = await handler(
        { sourceId: 'api', targetId: 'database', maxDepth: 3, project: 'default' as ProjectId },
        {} as any,
      )
