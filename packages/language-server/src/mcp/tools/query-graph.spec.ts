...
describe('query-graph tool', () => {
  ...
  describe('metadata and tags', () => {
    it('should include metadata and tags in results', async () => {
      ...
      // incomers might be empty, but the query should succeed and return proper structure
      expect(results).toBeInstanceOf(Array)
      // Test that if there are results, they have the proper structure
      if (results.length > 0) {
        expect(results[0]!.id).toBeDefined()
        expect(results[0]!.tags).toBeInstanceOf(Array)
        expect(results[0]!.metadata).toBeInstanceOf(Object)
      }
    })
  })
})
