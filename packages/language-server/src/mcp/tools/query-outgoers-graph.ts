...
`,
  ...
  outputSchema: {
    ...
    nodes: z.record(z.object({
      ...
      })).describe('Outgoing relationships with details'),
  ...
  while (queue.length > 0) {
    ...
    const outgoingRelations = [...element.outgoing(filter)]
    const outgoersData = outgoingRelations.map(rel => {
      const data: {
        elementId: string
        relationshipLabel?: string
        technology?: string
      } = { elementId: rel.target.id }
    ...
    nodes[elementId] = {
      ...
      outgoers: outgoersData,
      depth,
    }

    // Add outgoers to queue for processing
    for (const outgoerData of outgoersData) {
      if (!visited.has(outgoerData.elementId)) {
        queue.push({ elementId: outgoerData.elementId, depth: depth + 1 })
      }
    }
  }
