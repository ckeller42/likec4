...
describe('query-by-metadata tool', () => {
  ...
  describe('default matchMode', () => {
    it('should default to exact match when matchMode not specified', async () => {
      const { validate, services } = createTestServices()
      ...
      const [_name, _config, handler] = queryByMetadata(services.likec4.LanguageServices)
      const result = await handler(
        { key: 'owner', value: 'platform-team', project: 'default' as ProjectId },
        {} as any,
      )
  ...
  describe('empty string values', () => {
    ...
    it('should accept empty string as valid search parameter', async () => {
      const { validate, services } = createTestServices()
      ...
      // The fix ensures that if (!searchValue) doesn't reject empty string
      // Previously, empty string was treated the same as undefined
      const result = await handler(
        { key: 'status', value: '', matchMode: 'exact', project: 'default' as ProjectId },
        {} as any,
      )

      // Should succeed without error (the key distinction is no error thrown)
      expect(result.structuredContent).toBeDefined()
    })
  })
})
